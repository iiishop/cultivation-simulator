...
var _last_seed := 0
var _last_height_grid := PackedFloat32Array()
var _last_terrain_grid := PackedInt32Array()
var _last_half := 0
...
## 温度修正：海拔温降 + 海洋调温（沿海更温和，内陆更极端）
const TEMPERATURE_NOISE_AMPLITUDE := 0.18
const TEMPERATURE_LAPSE_START := 0.10
const TEMPERATURE_LAPSE_SPAN := 0.45
const TEMPERATURE_LAPSE_MAX := 0.34
const COASTAL_MODERATION_DISTANCE := 28.0
const COASTAL_COOLING_STRENGTH := 0.08
const INLAND_EXTREME_STRENGTH := 0.05
...
func get_temperature(pos: Vector2) -> float:
	...
	var wobble := noise.get_noise_2d(pos.x * 0.025, pos.y * 0.025) * TEMPERATURE_NOISE_AMPLITUDE
	return clampf(base + wobble, 0.0, 1.0)


## 最终温度：基础纬度温度 + 海拔温降 + 海洋调温
func _temperature_with_modifiers(pos: Vector2, height_raw: float, dist_to_coast: int) -> float:
	var base := get_temperature(pos)

	var lapse_t := clampf((height_raw - TEMPERATURE_LAPSE_START) / TEMPERATURE_LAPSE_SPAN, 0.0, 1.0)
	var elevation_cooling := lapse_t * TEMPERATURE_LAPSE_MAX

	var coastal_mix := 1.0 / (1.0 + float(dist_to_coast) / COASTAL_MODERATION_DISTANCE)
	var coastal_target := 1.0 - _latitude(pos)
	var coastal_adjust := (coastal_target - base) * COASTAL_COOLING_STRENGTH * coastal_mix

	var inland_factor := 1.0 - coastal_mix
	var inland_target := clampf((1.0 - _latitude(pos) - 0.5) * 2.0, -1.0, 1.0)
	var inland_adjust := inland_target * INLAND_EXTREME_STRENGTH * inland_factor

	return clampf(base - elevation_cooling + coastal_adjust + inland_adjust, 0.0, 1.0)
...
func get_height_meters_at_cell(coords: Vector2i) -> float:
	if _last_height_grid.is_empty():
		var raw_fallback := get_heightv(Vector2(coords))
		return (raw_fallback - SEA_LEVEL_RAW) * METERS_PER_RAW_UNIT
...
func get_terrain_at_cell(coords: Vector2i) -> int:
	if not _last_terrain_grid.is_empty():
		if coords.x < -_last_half or coords.x >= _last_half or coords.y < -_last_half or coords.y >= _last_half:
			return -1
		return _last_terrain_grid[_cell_idx(coords.x, coords.y, _last_half)]
...
func get_last_generation_report() -> Dictionary:
	return _last_generation_report.duplicate(true)
...
func generate(size: int) -> void:
	...
	noise_dry.seed = _last_seed + 2
	noise_ridge.seed = _last_seed + 10
	noise_chain.seed = _last_seed + 11
