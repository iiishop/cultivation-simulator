...
## 地形生成器：使用 FastNoiseLite + 衰减生成海陆与气候，支持河流
## 使用 TileMapLayer（Godot 4.x 推荐），替代已弃用的 TileMap
## 文档：godot-epub-docs TileMapLayer.set_cell(coords, source_id, atlas_coords, alternative_tile)
...
func generate(size: int) -> void:
	...
	for _i in river_count:
		...
		if is_water_cell(Vector2i(x, y)):
			continue
		generate_river(x, y)
	# 文档：TileMapLayer 更新在帧末批处理，update_internals() 立即刷新显示
	update_internals()
